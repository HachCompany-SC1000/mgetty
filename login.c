...
//#define IDLE_TIME
#define IDLE_TIME_ARG1 "idle" //
#define IDLE_TIME_ARG2 "600" // 1200 = 20 minutes
...
fallthrough:
...
#ifdef IDLE_TIME
    /*idle time*/
    argv[argc++] = IDLE_TIME_ARG1;
    argv[argc++] = IDLE_TIME_ARG2;
#endif
    /* terminate list */
    argv[argc] = NULL;

    //-------------------------------------------------------------------------------------
    //Aenderung fuer V 3.0 unterschiedliche options dateien für CSD und GPRS
    //Fehlt die Angabe fur das option file fuer die CSD Verbindung in 
    //login.conf, so wird die option Datei /etc/ppp/options_csd automatisch
    //beim Aufrufen von pppd mit uebergeben
    //calling login: cmd='/usr/X11R6/bin/pppd', argv[]='pppd ttySC0 38400 file /etc/ppp/options_csd xonxoff'
    //calling login: cmd='/usr/X11R6/bin/pppd', argv[]='pppd ttySC0 38400 xonxoff file /etc/ppp/options_csd'

    for( i=0; i<argc; i++)
     {if(strcmp(argv[i],"file") == 0)
       {lprintf( L_NOISE, "Found option file entry: %s %s -> Nothing to do", argv[i] , argv[i+1] );
        OptionsFile=1;
	break;
       }
     }
    ...
    if(OptionsFile == 0)
     {argv[argc]="file";
      argc++;
      argv[argc]="/etc/ppp/options_csd";
      argc++;
      lprintf( L_NOISE, "No option file found -> Add argv[%d]=%s , argv[%d]=%s",argc-2,"file",argc-1,"/etc/ppp/options_csd");
      /* terminate list */
      argv[argc] = NULL;
     }
