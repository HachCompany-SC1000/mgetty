...
#include <sys/types.h>
  ...
  struct termios		settings, old_tty_settings;
	...
	lprintf( L_NOISE, "### TIO RESET! (Loese Verklemmungen)###");
	bzero(&settings, sizeof(settings));
	cfsetispeed(&settings, B4800); // 25.03.2009.B115200 -> B4800
	cfsetospeed(&settings, B4800); // 25.03.2009.B115200 -> B4800
	...
	if((fd = open("/dev/ttySC0", O_RDWR | O_NOCTTY | O_NDELAY)) < 0) {
		lprintf( L_NOISE, "### TIO RESET: Error open device! ###");
		exit(0);
	}

	if(tcgetattr(fd, &old_tty_settings) < 0) {
		lprintf( L_NOISE, "### TIO RESET: Error tcgetattr()! ###");
	}
	if(tcsetattr(fd, TCSANOW, &settings) < 0) {
		lprintf( L_NOISE, "### TIO RESET: Error tcsetattr())! ###");
	}

	// close
	if(tcsetattr(fd, TCSANOW, &old_tty_settings) < 0)
	 lprintf( L_NOISE, "### TIO RESET: Error Zurueckschreiben tcsetattr()! ###");
